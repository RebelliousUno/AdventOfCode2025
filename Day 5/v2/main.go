...
func sortRanges(r RangeList) RangeList {
	slices.SortFunc(r.ranges, func(a, b Range) int {
		if a.min < b.min {
			return -1
		}
		if a.min > b.min {
...
func compressRanges(r RangeList) RangeList {
	compressedRange := make([]Range, 0)
	currentMin := r.ranges[0].min
	currentMax := r.ranges[0].max
	for _, v := range r.ranges[1:] {
		if v.min <= currentMax {
			// Overlaps: extend currentMax if needed
			if v.max > currentMax {
				currentMax = v.max
			}
		} else {
			// No overlap: flush current, start new
			compressedRange = append(compressedRange, Range{currentMin, currentMax})
			currentMin = v.min
			currentMax = v.max
		}
	}
	//get the final range
	compressedRange = append(compressedRange, Range{currentMin, currentMax})
	return RangeList{compressedRange}
}
...
func main() {
	...
	if errIngredients != nil {
		zLog.Fatal().Err(errRangeList).Msg("Problem with Ingredients List")
	}
	fresh := countFreshIngredients(rangeList, ingredientsList)
	zLog.Info().Msgf("Count of fresh Ingredients %d", fresh)

	sortedRanges := sortRanges(rangeList)
	zLog.Info().Msgf("sorted Range %v", sortedRanges)

	compressed := compressRanges(sortedRanges)
	zLog.Info().Msgf("Compressed Range %v", compressed)
	count := countRanges(compressed)
	zLog.Info().Msgf("Count of Ids %v", count)
}
